...
def process(filename):
    ...
    if len(glob.glob(f"{output}/**/{os.path.basename(filename)}")) != 0:
        ...
        try:
            ...
            while True:
                if k == 50:
                    ...
                    try:
                        ...
                        json_response = requests.post(url, data=data, headers=headers,timeout=60)
            ...
            detections.append([os.path.basename(filename),'Error',99,filename,''])
        return detections
    ...
    with open(f'{opt.output}/detections.csv', 'a',newline='') as file:
        ...
        with closing(multiprocessing.Pool(processes=num_workers)) as pool:
            ...
            for detection in detections:
                ...
                elif detection[0][2] != 0 and detection[0][2] != 99:
                    detection_count = detection_count + 1
                elif detection[0][2] == 99:
                    error_count = error_count + 1
                else:
                    empty_count = empty_count + 1
                pbar_text = f'Found {detection_count} objects in {image_count} images. {empty_count} empty images. {error_count} couldnt be processed'
                if detection!='Processed':
